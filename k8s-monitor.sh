...
monitor_deployments() {
    ...
    if deployments=$(kubectl get deployments -n "$NAMESPACE" -l app="$APP_NAME" --no-headers 2>/dev/null) && [ -n "$deployments" ]; then
        echo -e "${WHITE}NAME${NC}\t\t${WHITE}READY${NC}\t${WHITE}UP-TO-DATE${NC}\t${WHITE}AVAILABLE${NC}\t${WHITE}AGE${NC}"
        kubectl get deployments -n "$NAMESPACE" -l app="$APP_NAME" --no-headers | while read -r name ready uptodate available age; do
            echo -e "${CYAN}$name${NC}\t\t$(format_status "$ready")\t$(format_number "$uptodate" "")\t\t$(format_status "$available")\t\t${PURPLE}$age${NC}"
        done
    else
        ...
        if [ -n "$deployments" ]; then
            ...
            echo -e "${RED}❌ No deployments found for app: ${BOLD}$APP_NAME${NC}"
        fi
    fi
}
...
monitor_replicasets() {
    ...
    if replicasets=$(kubectl get replicasets -n "$NAMESPACE" -l app="$APP_NAME" --no-headers 2>/dev/null) && [ -n "$replicasets" ]; then
        ...
        if [ -n "$replicasets" ]; then
            ...
            echo -e "${RED}❌ No replicasets found for app: ${BOLD}$APP_NAME${NC}"
        fi
    fi
}
...
monitor_pods() {
    ...
    if pods=$(kubectl get pods -n "$NAMESPACE" -l app="$APP_NAME" --no-headers 2>/dev/null) && [ -n "$pods" ]; then
        ...
        if [ -n "$pods" ]; then
            ...
            echo -e "${RED}❌ No pods found for app: ${BOLD}$APP_NAME${NC}"
        fi
    fi
}
...
monitor_services() {
    ...
    if services=$(kubectl get services -n "$NAMESPACE" -l app="$APP_NAME" --no-headers 2>/dev/null) && [ -n "$services" ]; then
        ...
        if [ -n "$services" ]; then
            ...
            echo -e "${RED}❌ No services found for app: ${BOLD}$APP_NAME${NC}"
        fi
    fi
}
...
monitor_events() {
    ...
    if [ -n "$events" ]; then
        ...
        echo -e "${BLUE}ℹ️  No recent events found for app: ${BOLD}$APP_NAME${NC}"
    fi
}
...
monitor_pod_logs() {
    ...
    if pod_names=$(kubectl get pods -n "$NAMESPACE" -l app="$APP_NAME" -o jsonpath='{.items[*].metadata.name}' 2>/dev/null) && [ -n "$pod_names" ]; then
        for pod in $pod_names; do
            echo
            echo -e "${PURPLE}🔍 Logs for pod: ${BOLD}$pod${NC}"
...
print_usage() {
    echo "Usage: $0 [APP_NAME] [NAMESPACE] [REFRESH_INTERVAL]"
    echo "  APP_NAME: Application name to monitor (default: mlflow)"
    echo "  NAMESPACE: Kubernetes namespace (default: ml-ops)"
    echo "  REFRESH_INTERVAL: Refresh interval in seconds (default: 10)"
    echo
    echo "Example: $0 myapp production 5"
    echo
...
main() {
    ...
    echo "App: $APP_NAME | Namespace: $NAMESPACE | Refresh: ${REFRESH_INTERVAL}s"
    ...
    while true; do
        ...
        echo -e "${BOLD}${GREEN}🚀 Application: ${CYAN}$APP_NAME${NC} ${BOLD}${BLUE}| 🎯 Namespace: ${CYAN}$NAMESPACE${NC} ${BOLD}${PURPLE}| ⏱️  Refresh: ${CYAN}${REFRESH_INTERVAL}s${NC}"
